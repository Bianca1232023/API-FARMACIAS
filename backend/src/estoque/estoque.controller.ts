...
export class EstoqueController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de remédios com estoque baixo.',
  })
  @ApiResponse({
    status: 204,
    description: 'Não tem nennhum remédio com estoque baixo.',
  })
  async findLowStock() {
  ...
  @ApiResponse({
    status: 404,
    description: 'Estoque não encontrado ou removido pois a quantidade zerou.',
  })
  @ApiParam({ name: 'id', type: Number })
  ...
  @ApiResponse({
    status: 200,
    description: 'Estoque atualizado parcialmente com sucesso.',
    type: Estoque,
  })
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  @ApiResponse({
  ...
  async patch(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: Partial<UpdateEstoqueDto>,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Estoque da farmácia encontrados.',
    type: [Estoque],
  })
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Estoque de remedios encontrados.',
    type: [Estoque],
  })
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Estoque atualizado com sucesso após a doação.',
    type: Estoque,
  })
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  @ApiParam({ name: 'farmaciaId', type: Number })
  ...
  @ApiOperation({
    summary: 'Verificar se um remédio está disponível no estoque',
  })
  @ApiResponse({
    status: 200,
    description: 'Disponibilidade do remédio verificada com sucesso.',
    type: Boolean,
  })
